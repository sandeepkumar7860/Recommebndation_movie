...
# CONFIG
# =============================
API_BASE = "https://recommebndation-movie-4.onrender.com" or "http://127.0.0.1:8000"  # change manually if needed
TMDB_IMG = "https://image.tmdb.org/t/p/w500"
...
.movie-title { font-size: 0.9rem; height: 2.3rem; overflow: hidden; }
</style>
""", unsafe_allow_html=True)
...
def api_get(path, params=None):
    try:
        res = requests.get(f"{API_BASE}{path}", params=params, timeout=20)
...
# POSTER GRID
# =============================
def poster_grid(movies):
    cols = st.columns(6)
    for i, m in enumerate(movies):
        with cols[i % 6]:
            if m.get("poster_url"):
                st.image(m["poster_url"], width="stretch")  # ✅ FIXED
            else:
                st.write("No Image")

            if st.button("Open", key=f"{i}_{m.get('tmdb_id')}"):
...
if st.session_state.view == "home":
    ...
    if query:
        ...
        home = api_get("/home", {"category": "trending", "limit": 24})
        if home:
            poster_grid(home)
...
# =============================
    ...
    if not data:
        ...
        with col1:
            if data.get("poster_url"):
                st.image(data["poster_url"], width="stretch")  # ✅ FIXED
